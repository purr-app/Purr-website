...
for name in ['hero','tracing','jpath']:
...
# Final-frame posters match each recording; demos play once and keep that frame.
for name,time in [('hero','5.30'),('tracing','3.80'),('jpath','5.45')]:
 png=out/(name+'-end.png')
 subprocess.run([ff,'-y','-ss',time,'-i','assets/'+name+'.mp4','-frames:v','1',str(png)],capture_output=True,check=True)
 Image.open(png).save(out/(name+'-end.webp'),quality=90)
 png.unlink()
